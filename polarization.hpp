...
#pragma omp parallel for private(r,c)
    ...
    //replace values less than low or greater than high with NaN. Returns the number of replaced values.
    DLL_PUBLIC uint32_t mask_low_high(const size_t n, const float* raw, float low, float high, float* out);
    DLL_PUBLIC uint32_t mask_low(const size_t n, const float* raw, float low, float* out);
    DLL_PUBLIC uint32_t mask_high(const size_t n, const float* raw, float high, float* out);
